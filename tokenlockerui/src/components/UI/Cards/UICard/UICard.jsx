    ...
    <div className={`${styles.card}`}>
      <div className={styles.header}>
        ...
        <span className={styles.indicator}
          style={{
            backgroundColor: `${remainingDays === 0 ? 'green' :
                elapsedDays < remainingDays ? 'red' :
                  elapsedDays >= remainingDays ? 'orange' : ""}`,
            boxShadow: `${remainingDays === 0 ? '0px 0px 20px green' :
      ...
      <span style={{color: isReleased ? "lightgreen" : "orange"}}>{isReleased ? "Released" : "~ Locked"}</span>
      {/* </Link> */}
    </div>
  );
}
